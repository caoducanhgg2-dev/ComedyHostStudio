...
Clean wrapper around the preserved 5.5.4 core. SRT-only routes to the factual
visual writer. The runtime uses adaptive RAM-safe GPU batches, explicitly hands
VRAM from Vision to Writer, and frees the final Writer after each video so the
next video starts clean without restarting Ollama.
"""
...
VERSION = "1.1.0-beta5.5.6-visual-quality-fast"
...
def _plan(self, story, duration):
    story = story or {}
    return {
        "version": "5.5.6-visual-quality-fast",
...
def _writer(self, observations, story, duration, language, transcript=None, dialogue_mode=False, creative_plan=None):
    # Vision is no longer needed after the evidence map exists. Explicit unload is
    # faster and safer than making Ollama evict it while the 8B Writer is loading.
    handoff = prepare_writer_phase(self.r)
    if handoff.get("attempted") and not handoff.get("remaining"):
        core.emit("setup", "5.5.6 Fast: đã nhường RAM/VRAM từ Visual Brain sang Writer.")
...
def _fast_vi_translation(self, script, source_language, creative_plan=None):
    """Fast review translation: two larger batches for a typical ~45-caption clip.

    The old extra AI summary call is intentionally removed. The user now uses the
    factual SRT/Visual_Context as source material for a separate ChatGPT rewrite,
    so Vietnamese is only a quick accuracy check.
    """
    rows = []
    batch_size = 24
    ...
    def translate_batch(batch, start):
        ...
        return self.r.chat("Bạn dịch phụ đề video sang tiếng Việt tự nhiên. Chỉ trả JSON.", prompt,
    ...
    core.write_json(self.out/"Vietnamese_QA.json",{
        "version":"5.5.6-fast","source_language":source_language,"captions":len(rows),
        "issues":issues,"issue_count":len(issues),"glossary":glossary,
        "policy":"Fast meaning-first Vietnamese review translation; no extra AI summary pass."
    })
    return len(rows)
...
def _stable_execute(self):
    ...
    if not report.get("skipped"):
        if report.get("remaining"):
            core.emit("warning", "5.5.6: model video trước chưa giải phóng hoàn toàn: " + ", ".join(report["remaining"]) + ".")
        elif report.get("attempted"):
            core.emit("setup", "5.5.6 Fast: GPU/RAM đã sạch trước video tiếp theo.")
    try:
        return _original_execute(self)
    finally:
        # Free Writer immediately. Ollama server remains alive, so the next video
        # avoids stale-model RAM pressure without paying a full server restart.
        finish_job(self.r)
