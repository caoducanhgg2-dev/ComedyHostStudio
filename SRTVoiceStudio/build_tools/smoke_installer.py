"""Clean install and upgrade the exact 1.0 artifact; real offline installed acceptance."""
import os
import sys
...
installer=root/'installer-output'/'SRTVoiceStudio_Setup_1.1.0.exe'
baseline=root/'baseline-installer'/'SRTVoiceStudio_Setup.exe'
expected='108d022d41ddc53fd089b01ce0bfb6edeced385504829f73655cf3ca8c13f31a'
...
results={'version':'1.1.0','baseline_sha256':expected,'scenarios':{}}
...
for scenario in ('upgrade','clean'):
    ...
    try:
        if scenario=='upgrade':
            ...
            old=registry();assert old['DisplayVersion']=='1.0.0',old
            # No /DIR for the upgrade: Inno must find the existing AppId/location.
            install(installer)
        else:
            install(installer)
        state=registry()
        assert state['DisplayVersion']=='1.1.0' and state['DisplayName']=='SRT Voice Studio',state
        ...
        for i,program in enumerate([exe,dest/'_internal'/'bin'/'ffmpeg.exe',dest/'_internal'/'bin'/'ffprobe.exe']):
            name=rule+str(i)
