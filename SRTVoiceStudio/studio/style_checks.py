    ...
    with tempfile.TemporaryDirectory(prefix='job-', dir=workspace()) as folder:
        ...
        for emotion in EMOTIONS:
            for intensity in LEVELS:
                try:
                    b, tempo = processor.apply_emotion(
                        original.samples, original.rate, emotion, intensity
                    )
                except Exception as exc:
                    raise RuntimeError(
                        f'Emotion preset failed: {emotion} / {intensity}: {exc}'
                    ) from exc
                assert len(b) > 0 and np.isfinite(b).all() and .9 <= tempo <= 1.2
                if emotion != 'Natural':
                    assert len(b) != len(original.samples) or not np.allclose(
                        b, original.samples, atol=1e-5
                    )
        ...
        for effect in EFFECTS:
            for strength in LEVELS:
                try:
                    b = processor.apply_effect(
                        original.samples, original.rate, effect, strength
                    )
                except Exception as exc:
                    raise RuntimeError(
                        f'Voice FX preset failed: {effect} / {strength}: {exc}'
                    ) from exc
        ...
        expected, record = fit_processed(
            b.samples, b.rate, slot, styled, cache.emotion_tempo, folder, cancel
        )
        ...
        checks.extend([
            'Preview B: OK (same immutable A)',
            'Timeline Preview C: OK (production fit)',
        ])
